    ...
    <>
      <div className="my-3 d-flex justify-content-center">
        <div className="card" style={{ width: "18rem" }}>
          <span
            className={`position-absolute top-0 start-50 translate-middle badge rounded-pill text-bg-${badgeColor}`}
            style={{ zIndex: "1" }}
          >
            {source ? source : "World"}
          </span>
          <img
            src={
              !imageUrl
                ? "https://storage.googleapis.com/gweb-uniblog-publish-prod/images/HNW_illustrations_v3-29.max-1000x1000.png"
                : imageUrl
            }
          ...
          <div className="card-body">
            ...
            <p className="card-text">
              <small className="text-muted">
                {author ? titleCase(author) : "By Anonymous"} on {formatedDate}
              </small>
            </p>
            <a
              href={newsUrl}
              target="_blank"
              rel="noreferrer"
              className="btn btn-sm btn-secondary readBtn"
            >
              Read More
            </a>
          </div>
